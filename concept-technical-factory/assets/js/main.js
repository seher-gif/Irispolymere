/* ==========================================================================
   Iris Polymere — Technical Factory Grid
   Site behaviour: mobile accordion nav, language switching, form
   validation, certificate modal, scroll-reveal, applications/hero-strip
   hover-preview. No external dependencies. Desktop mega menu and hero
   product tabs open/switch via CSS (:hover / :focus-within) where
   possible — this file only wires up state that CSS cannot express.
   ========================================================================== */
(function () {
  "use strict";
  ...
  /* ---------------------------------------------------------------------
     7. Applications — hover/focus updates the adjacent preview image
     ------------------------------------------------------------------- */
  function initAppPreview() {
    document.querySelectorAll("[data-app-group]").forEach(function (group) {
      var rows = group.querySelectorAll("[data-app-target]");
      var images = group.querySelectorAll("[data-app-panel]");
      if (!rows.length || !images.length) return;

      function activate(targetId) {
        rows.forEach(function (row) {
          row.classList.toggle("is-active", row.getAttribute("data-app-target") === targetId);
        });
        images.forEach(function (img) {
          img.classList.toggle("is-active", img.getAttribute("data-app-panel") === targetId);
        });
      }

      rows.forEach(function (row) {
        var id = row.getAttribute("data-app-target");
